...
- **DC-DC charging** — a Buck-Boost Charge Controller accepts 10–30V DC input via an Anderson Powerpole panel connector, configured for 14.6V CC/CV (4S LiFePO4). The charger negative output connects to the system/load side of the main current shunt (not directly to the battery) so telemetry can measure charge current. The charger must be shielded or RF-quiet to avoid HF interference.
