...
- **DC-DC charging** — an LTC3780 10A Buck-Boost Converter accepts 10–30V DC input via an Anderson Powerpole panel connector, physically calibrated to 14.6V CV / 5A CC (4S LiFePO4). The output is filtered by an LC Pi-Filter (1000µF + 0.1µF caps across OUT terminals, then both wires through solid ferrite beads for common-mode rejection) followed by a 15A inline fuse. Input has a 15A inline fuse. The charger negative output connects to the system/load side of the main current shunt (not directly to the battery) so telemetry can measure charge current. The bare PCB is housed in a 3D-printed ABS enclosure lined with copper tape as a Faraday cage.
